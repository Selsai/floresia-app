...
import { ChevronLeft as FiChevronLeft, ChevronRight as FiChevronRight, Check as FiCheck, ShoppingCart as FiShoppingCart, Plus as FiPlus, Minus as FiMinus, X as FiX, Cake, Gem, Heart, UserRound, HandHeart, HeartHandshake, Flower2 } from 'lucide-react';
import { flowersApi } from '../../services/api';
import { ComposerSkeleton } from '../../components/loading/LoadingSkeleton';
import './CustomBouquet.css';

// ─── Données statiques (occasions et rubans ne viennent pas de la base) ──────

const OCCASIONS = [
  { id: 'birthday',  name: 'Anniversaire',  icon: <Cake size={24} strokeWidth={1.7} aria-hidden="true" /> },
  { id: 'wedding',   name: 'Mariage',        icon: <Gem size={24} strokeWidth={1.7} aria-hidden="true" /> },
  { id: 'valentine', name: 'Saint-Valentin', icon: <Heart size={24} strokeWidth={1.7} aria-hidden="true" /> },
  { id: 'mother',    name: 'Fête des mères', icon: <UserRound size={24} strokeWidth={1.7} aria-hidden="true" /> },
  { id: 'thanks',    name: 'Remerciement',   icon: <HandHeart size={24} strokeWidth={1.7} aria-hidden="true" /> },
  { id: 'love',      name: 'Déclaration',    icon: <HeartHandshake size={24} strokeWidth={1.7} aria-hidden="true" /> },
];

const RIBBONS = [
  { id: 'red',        name: 'Rouge',       color: 'var(--ribbon-red)' },
  { id: 'pink',       name: 'Rose',        color: 'var(--ribbon-pink)' },
  { id: 'white',      name: 'Blanc',       color: 'var(--white)' },
  { id: 'beige',      name: 'Beige',       color: 'var(--ribbon-beige)' },
  { id: 'gold',       name: 'Doré',        color: 'var(--ribbon-gold)' },
  { id: 'blue',       name: 'Bleu',        color: 'var(--ribbon-blue)' },
  { id: 'khaki',      name: 'Vert kaki',   color: 'var(--ribbon-khaki)' },
  { id: 'multicolor', name: 'Multicolore', color: 'linear-gradient(90deg,var(--ribbon-multi-red),var(--ribbon-multi-yellow),var(--ribbon-multi-green),var(--ribbon-blue))' },
];
                    ...
                    {secondaryFlowers.map(flower => (
                      ...
                      >
                        ...
                        <div className="option-info">
                          ...
                          <span className="option-price">+{formatPrice(flower.price)} / tige</span>
                        </div>
                      </button>
                    ))}
                  </div>
              ...
              <div className="step-navigation">
                ...
                {currentStep < 4 ? (
                  ...
                  <button onClick={addBouquetToCart} className="btn-add-cart">
                    <FiShoppingCart /> Ajouter au panier — {formatPrice(calculatePrice())}
                  </button>
                )}
              </div>
                  ...
                  <div className="preview-visual">
                    {bouquetConfig.selectedFlowers.length > 0 ? (
                      ...
                      <div className="preview-placeholder">
                        <Flower2 size={35} strokeWidth={1.3} aria-hidden="true" />
          ...
          return (
            <div key={flower.id} className={`flower-item ${selected ? 'selected' : ''}`}>
              ...
              <div className="flower-item-info">
                ...
                <span className="flower-price">{formatPrice(flower.price)} / tige</span>
              </div>
              {selected ? (
